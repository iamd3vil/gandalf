...
| --- | --- | --- |
| `required` | value is its zero value | Supports strings; `int`, `int16`, `int32`, `int64`, `float32`, and `float64`; slices; and other nil-able values. |
| `min:n` | value or length is less than `n` | Inclusive lower bound (`>= n`). Strings and slices use `len`. |
...
| `regexp:pattern` | string does not match the pattern | The pattern is compiled once in generated package-level code. Single quotes around the pattern are removed. |
...
- define `Validate() error` on a pointer receiver for every struct with validation tags;
- return the first validation error;
- precompile regular expressions as package-level variables; and
...
Gandalf is an early-stage generator and currently has a deliberately small parser:

- it handles simple named field types and slices of simple named types;
- embedded fields, arrays, maps, qualified types, and more complex type expressions are not fully supported;
- regular-expression constraints are intended for string fields, and patterns cannot contain spaces or `:` characters; and
- malformed constraints may cause generation to fail rather than returning a friendly diagnostic.
